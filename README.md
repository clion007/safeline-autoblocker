...
wget https://raw.gitmirror.com/clion007/safeline-auto-blocker/main/safeline_auto_blocker.py
```
...
| SAFELINE_LOG_FILE | 雷池安全日志文件路径 | /var/log/safeline/security.log |
| ATTACK_TYPES_FILTER | 攻击类型过滤，多个ID用逗号分隔 | (空，监控所有类型) |
| QUERY_INTERVAL | API查询间隔（秒） | 60 |
| MAX_LOGS_PER_QUERY | 每次查询的最大日志数量 | 100 |
| FOLLOW_LOG_ROTATION | 是否跟踪日志轮转 | true |
| DEBUG_MODE | 是否启用调试模式 | false |

此外，配置文件还包含 `[TYPE_GROUP_MAPPING]` 部分，用于配置不同攻击类型ID对应的IP组：
...
| --monitor | 日志文件监控模式 | `python3 safeline_auto_blocker.py --monitor` |
...
```bash
sudo python3 /opt/safeline/scripts/safeline_auto_blocker.py --monitor
```
